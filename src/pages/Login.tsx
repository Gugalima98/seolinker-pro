...
const Login = () => {
  ...
  const { login, loading, signInWithGoogle, resetPassword } = useAuth();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (success) {
      toast({ title: "Login realizado com sucesso!", description: "Bem-vindo de volta à plataforma." });
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      toast({ title: "Sucesso", description: "Se o e-mail estiver correto, um link para recuperação de senha foi enviado." });
      setView('login');
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-light via-background to-muted flex items-center justify-center p-4">
      <div className="w-full max-w-md animate-fade-in">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold gradient-text">8links</h1>
          ...
          <Card className="shadow-elegant border-0 backdrop-blur-sm bg-card/95">
            ...
            <form onSubmit={handleLogin}>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-right text-sm">
                    <a href="#" onClick={(e) => { e.preventDefault(); setView('forgotPassword'); }} className="text-muted-foreground hover:text-primary transition-colors">
                      Esqueceu sua senha?
                    </a>
                  </div>
                </div>
              ...
              <CardFooter className="space-y-4 flex flex-col">
                <Button type="submit" className="w-full bg-gradient-to-r from-primary to-primary-hover hover:shadow-glow transition-all duration-300" disabled={loading}>
                  {loading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Entrando...</> : 'Entrar'}
                </Button>
                {/* Botão do Google desativado temporariamente
                <Button variant="outline" className="w-full flex items-center gap-2" onClick={signInWithGoogle} disabled={loading}>
            ...
            <form onSubmit={handleForgotPassword}>
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Mail className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                    <Input id="email-forgot" type="email" placeholder="seu@email.com" value={email} onChange={(e) => setEmail(e.target.value)} className="pl-10" required />
                  </div>
                </div>
              </CardContent>
              <CardFooter className="flex flex-col space-y-4">
                ...
                <Button variant="link" onClick={() => setView('login')} className="text-muted-foreground">Voltar para o Login</Button>
              </CardFooter>
            </form>
          </Card>
        )}
